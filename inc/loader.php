<?php
// Ovde ucitavamo sve klase koje su nam potrebne. Nemoguce je sve to odraditi u funckcijama jer taj fajl ne koristimo svugde. Pa da nebude gresaka tu sam razdvojio
// Podesavanja
require_once("configdb.php");
// DB KLASA
include_once("mysqli.class.php");
// Security ciscenje promenjivih koje dolaze preko post.get.request
include_once("request.php");
$request = new Request();
// Klasa za logovanje
include_once("log.php");
$log = new Log("general.log");
// Mail Klasa
...
$log->File("sms.log");
	...
	// Broj mora biti bez zaraza, crtica i praznih mesta!!!!
	$brojevi = array($phone);
	If($data['lang']=='de'){
		$poruka = "DE:Chek mail to confirm taxi order from ".HOST.". If You dont see mail look in SPAM folder.";
	}else{
		$poruka = "Chek mail to confirm taxi order from ".HOST.". If You dont see mail look in SPAM folder.";
	}
	
	$smsClient = new WebSmsCom_Client(SMSUSER, SMSPASS, SMSGTW);
// Pripremanje poruke
	$message = new WebSmsCom_TextMessage($brojevi,$poruka);
// Slanje poruke
	$response = $smsClient->send($message, SMSMAXPERMESSAGE, SMSTEST);
	$res= $response->getStatusMessage();

	if($res=='OK'){
	    $log->write("SMS succesful sent ".$brojevi[0].": ".$poruka."");
	//    $return = true;
	}else{
	    $log->write("ERROR:".$res."| sending sms ".$brojevi[0].":t ".$poruka."");
	//    $return = false;
	}
*/
...
$one                               = new Template('Syrenna', '1.1', 'assets'); // Name, version and assets folder's name

// Global Meta Data
$one->author                       = 'Syrenna';
$one->robots                       = 'noindex, nofollow';
$one->title                       	 = 'Syrenna - Admin Dashboard';
$one->description                 = 'Syrenna - Admin Dashboard';
...
$one->body_bg                      = 'assets/img/photos/photo25@2x.jpg';       // eg 'assets/img/photos/photo10@2x.jpg' Useful for login/lockscreen pages
...
$one->main_nav                     = array(
    array(
	 ...
	 array(
        'name'  => '<span class="sidebar-mini-hide">CMS</span>',
        'type'  => 'heading'
    ), 
	array(
        'name'  => '<span class="sidebar-mini-hide">Live Shows</span>',
        'icon'  => 'si si-list',
        'sub'   => array(
            array(
                'name'  => 'List',
	...
	array(
        'name'  => '<span class="sidebar-mini-hide">Gallery</span>',
        'icon'  => 'si si-grid',
        'sub'   => array(
			array(
                'name'  => 'List',
            ...
            array(
                'name'  => 'Add photo',
                'url'   => 'gallery2_add.php',
            )
        )
    ),
	array(
        'name'  => '<span class="sidebar-mini-hide">Content</span>',
        'icon'  => 'si si-note',
        'sub'   => array(
			array(
                'name'  => 'Bio',
    ...
    array(
        'name'  => '<span class="sidebar-mini-hide">ETC</span>',
        'type'  => 'heading'
    ),
	 array(
        'name'  => '<span class="sidebar-mini-hide">Change Password</span>',
        'icon'  => 'si si-lock',
        'url'   => 'change_pass.php'
    ),
	 array(
        'name'  => '<span class="sidebar-mini-hide">Backup</span>',
        'icon'  => 'si si-refresh',
        'url'   => 'backup.php'
    ),
    array(
